...
SUPPORTED_PREDICTION_FORMATS = [fmt.lower() for fmt in ["json"]] # JSON is always supported even when not mentioned

def send_payload(sock, payload_obj, wire_fmt):
    
    """
    Helper to pack and send JSON, prefix with 4-byte length, and send.
...
def recv_message_loop(client_socket):
    ...
    while connection_active:
        ...
        # Handle unsupported `interaction_matrix` and `track` readout
        if readout_type == "interaction_matrix" or readout_type == "track":
            print(f"DREAM-RNN cannot handle '{readout_type}' readout type. Exiting gracefully!")
            json_return_error = {'bad_prediction_request': 
                [f"DREAM-RNN cannot process '{readout_type}' readout type."]}
        ...
        # --- Upstream and downstream flanking sequences, if provided by the evaluator ---
